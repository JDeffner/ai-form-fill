...
answer of `male` can select the *Female* radio/option if it appears first in the DOM. Same class
...
suite**, whose tests `return` early when Ollama is down — i.e. they *silently pass green without
testing anything*. False confidence + confusing for contributors.
...
*(If FR-08 is renegotiated in the thesis, the better long-term setup is untracking `dist/` and
attaching build artifacts to GitHub Releases — noted as future work, do not do it now.)*
...
| *(new, Phase 2a)* | `lib/providers/http.ts` (shared fetch helper — lives with its only consumers) |
...
| `getFieldIdentifier` | *(deleted — replaced by `field.key`, see 2c)* | |
...
1. `.gitignore`: **remove the `tests/` line**; add `dist/` is *not* added (see D3) but add
...
4. **Fix silent-pass integration tests**: replace the `if (!isOllamaAvailable) return;` pattern
   with a `beforeAll` that *throws* a clear "Ollama not reachable at http://localhost:11434 —
   start Ollama or skip via test:integration" error, or use `describe.skipIf(!env.OLLAMA)` so
   skipped tests are *reported as skipped*, never green.
...
- Update `mock/*.mock.ts` to be plain passthrough proxies of the *standard* format (they become
