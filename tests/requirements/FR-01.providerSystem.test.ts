...
describe('FR-01: Provider System', () => {
  ...
  it('AC-2: Provider implementations exist for remote and local services', () => {
    ...
    expect(remoteProvider.getType()).toBe('remote');
  });
  ...
  // AC-5: All providers pass the same suite of tests when given equivalent prompts.
  // (Verified by all providers implementing the same interface - see AC-1, AC-2.)

  // AC-6: The interface includes methods for sending chat requests and checking availability.
